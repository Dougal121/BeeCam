...
extern unsigned long postSunrise;
extern unsigned long numberSamples;
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
  ...
  else if (!strcmp(variable, "postsunrise"))
    setPostSunrise(val);
  else if (!strcmp(variable, "numbersamples"))
    setNumerSamples(val);
...
//  p += sprintf(p, "\"postsunrise\":%u", postSunrise);  
//  p += sprintf(p, "\"numersamples\":%u", numberSamples);  
	*p++ = '}';
	*p++ = 0;
...
/*
static esp_err_t updateget_handler(httpd_req_t *req)
{
  httpd_resp_set_type(req, "text/html");
  unsigned long l = strlen(updatePage);
  return httpd_resp_send(req, (const char *)updatePage, l);
}

static esp_err_t updatepost_handler(httpd_req_t *req)
{
  httpd_resp_set_type(req, "text/html");
  unsigned long l = strlen(updatePage);
  return httpd_resp_send(req, (const char *)updatePage, l);
...
/*
  httpd_uri_t updateget_uri = {
    .uri = "/update",
    .method = HTTP_GET,
    .handler = updateget_handler,
    .user_ctx = NULL};    

  httpd_uri_t updatepost_uri = {
    .uri = "/update",
    .method = HTTP_POST,
    .handler = updatepost_handler,
    .user_ctx = NULL};    
*/
...
//    httpd_register_uri_handler(camera_httpd, &updateget_uri);
//    httpd_register_uri_handler(camera_httpd, &updatepost_uri);
